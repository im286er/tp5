...
class User extends Admin
{
    ...
    public function role_add(){
        ...
        if(isset($_POST['dosubmit'])) {
            //根据表单提交的POST数据创建数据对象
            $model = $RoleDB->create(input('post.'));
            if($model) {
    ...
    public function role_edit(){
        ...
        if(isset($_POST['dosubmit'])) {
            //根据表单提交的POST数据创建数据对象
            $model = $RoleDB->update(input('post.'));
            if($model) {
    ...
    public function role_sort(){
        ...
        foreach ($sorts as $id => $sort) {
            $list[] = ['id' =>$id , 'sort' =>intval($sort)];
        }

        model('Role')->saveAll($list);
